// this soruce code was copied from https://github.com/esbuild/deno-esbuild/blob/main/wasm.js
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.17.11") {
        throw new Error(
          `Cannot start service: Host version "${"0.17.11"}" does not match binary version ${
            quote(binaryVersion)
          }`,
        );
      }
...
`);
              return i != -1 &&
                (console.log(c.substring(0, i)), c = c.substring(i + 1)),
                s.length;
            },
            write(n, s, i, a, h, u) {
...
`),
      ...
      async function tryToInstantiateModule(r, f) {
        if (r instanceof WebAssembly.Module) {
          return WebAssembly.instantiate(r, f.importObject);
        }
        const g = await fetch(r);
        if (!g.ok) throw new Error(`Failed to download ${JSON.stringify(r)}`);
        if (
          "instantiateStreaming" in WebAssembly &&
          /^application\/wasm($|;)/i.test(g.headers.get("Content-Type") || "")
        ) {
          return (await WebAssembly.instantiateStreaming(g, f.importObject))
            .instance;
        }
